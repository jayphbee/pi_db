...
fn test_fork() {
	...
	let _ = rt1.spawn(rt.alloc(), async move {
		*STORE_RUNTIME.write().await = Some(rt.clone());
		...
		// 创建一个用于存储元信息的表
		let _ = tr.alter(&Atom::from("logfile"), &Atom::from("./testlogfile/tabs_meta"), Some(meta)).await;
		let _ = tr.alter(&Atom::from("logfile"), &Atom::from("./testlogfile/hello"), Some(meta1)).await;
		...
		let _ = tr2.modify(vec![item1, item2, item3, item4], None, false).await;
		...
		let _ = tr3.fork_tab(Atom::from("logfile"), Atom::from("./testlogfile/hello"), Atom::from("./testlogfile/hello_fork"), tm).await;
...
fn test_load_data() {
	...
	let _ = rt1.spawn(rt.alloc(), async move {
		*STORE_RUNTIME.write().await = Some(rt.clone());
		...
		let meta1 = TabMeta::new(sinfo::EnumType::Str, sinfo::EnumType::Str);
		let _ = tr1.alter(&Atom::from("logfile"), &Atom::from("./testlogfile/hello"), Some(meta1.clone())).await;
		let _ = tr1.alter(&Atom::from("logfile"), &Atom::from("./testlogfile/hello_fork"), Some(meta1.clone())).await;
		let _ = tr1.alter(&Atom::from("logfile"), &Atom::from("./testlogfile/hello_fork2"), Some(meta1)).await;
		...
		let mut tr4 = mgr.transaction(true, Some(rt.clone())).await;
		let tm = TabMeta::new(sinfo::EnumType::Str, sinfo::EnumType::Str);
		let _ = tr4.fork_tab(Atom::from("logfile"), Atom::from("./testlogfile/hello_fork"), Atom::from("./testlogfile/hello_fork2"), tm).await;
		let _ = tr4.prepare().await;
		let _ = tr4.commit().await;
