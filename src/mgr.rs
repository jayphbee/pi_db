...
use ordmap::ordmap::{Entry, ImOrdMap, Keys, OrdMap};
use r#async::lock::mutex_lock::Mutex;
use r#async::rt::{multi_thread::MultiTaskRuntime, AsyncMap, AsyncRuntime};

use crate::db::{
    Bin, CommitResult, DBResult, Event, Filter, IterResult, KeyIterResult, RwLog, SResult, TabKV,
    TabMeta, TxCallback, TxState,
};
use crate::fork::{TableMetaInfo, ALL_TABLES};
use crate::log_file_db::{
    LogFileDB, LogFileDBSnapshot, LogFileMetaTxn, RefLogFileTxn, DB_META_TAB_NAME,
};
use crate::memery_db::{MemDB, MemDBSnapshot, MemeryMetaTxn, RefMemeryTxn};
...
impl DatabaseWareSnapshot {
    ...
    pub async fn list(&self) -> Box<dyn Iterator<Item = Atom>> {
        match self {
            DatabaseWareSnapshot::MemSnapshot(shot) => shot.list().await,
            DatabaseWareSnapshot::LogFileSnapshot(shot) => shot.list().await,
        }
    }
    ...
    pub async fn tab_info(&self, tab_name: &Atom) -> Option<TabMeta> {
        match self {
            DatabaseWareSnapshot::MemSnapshot(shot) => shot.tab_info(tab_name).await,
            DatabaseWareSnapshot::LogFileSnapshot(shot) => shot.tab_info(tab_name).await,
        }
    }
    ...
    pub fn check(&self, tab: &Atom, meta: &Option<TabMeta>) -> DBResult {
        match self {
            DatabaseWareSnapshot::MemSnapshot(shot) => shot.check(tab, meta),
            DatabaseWareSnapshot::LogFileSnapshot(shot) => shot.check(tab, meta),
        }
    }
    ...
    pub async fn alter(&self, tab_name: &Atom, meta: Option<TabMeta>) {
        match self {
            DatabaseWareSnapshot::MemSnapshot(shot) => shot.alter(tab_name, meta).await,
            DatabaseWareSnapshot::LogFileSnapshot(shot) => shot.alter(tab_name, meta).await,
        }
    }
    ...
    /**
    	...
    	* @returns
    	*/
    pub async fn tab_txn(
        &self,
        tab_name: &Atom,
        id: &Guid,
        writable: bool,
    ) -> SResult<Arc<DatabaseTabTxn>> {
        match self {
    ...
    pub async fn prepare(&self, id: &Guid) -> DBResult {
        match self {
            DatabaseWareSnapshot::MemSnapshot(shot) => shot.prepare(id).await,
            DatabaseWareSnapshot::LogFileSnapshot(shot) => shot.prepare(id).await,
        }
    }
    ...
    pub async fn commit(&mut self, id: &Guid) {
        match self {
            DatabaseWareSnapshot::MemSnapshot(shot) => shot.commit(id).await,
            DatabaseWareSnapshot::LogFileSnapshot(shot) => shot.commit(id).await,
        }
    }
    ...
    pub async fn rollback(&self, id: &Guid) {
        match self {
            DatabaseWareSnapshot::MemSnapshot(shot) => shot.rollback(id).await,
            DatabaseWareSnapshot::LogFileSnapshot(shot) => shot.rollback(id).await,
        }
    }
...
impl DatabaseWare {
    ...
    async fn tab_info(&self, tab_name: &Atom) -> Option<TabMeta> {
        match self {
            DatabaseWare::MemWare(memdb) => memdb.tab_info(tab_name).await,
            DatabaseWare::LogFileWare(logfiledb) => logfiledb.tab_info(tab_name).await,
        }
    }
    ...
    async fn list(&self) -> Box<dyn Iterator<Item = Atom>> {
        match self {
            DatabaseWare::MemWare(memdb) => memdb.list().await,
            DatabaseWare::LogFileWare(logfiledb) => logfiledb.list().await,
        }
    }
    ...
    fn timeout(&self) -> usize {
        match self {
            DatabaseWare::MemWare(memdb) => memdb.timeout(),
            DatabaseWare::LogFileWare(logfiledb) => logfiledb.timeout(),
        }
    }
}
...
impl DatabaseTabTxn {
    ...
    async fn get_state(&self) -> TxState {
        match self {
            DatabaseTabTxn::MemTabTxn(txn) => txn.get_state().await,
            DatabaseTabTxn::LogFileTabTxn(txn) => txn.get_state().await,
        }
    }
    ...
    pub async fn prepare(&self, timeout: usize) -> DBResult {
        match self {
            DatabaseTabTxn::MemTabTxn(txn) => txn.prepare(timeout).await,
            DatabaseTabTxn::LogFileTabTxn(txn) => txn.prepare(timeout).await,
        }
    }
    ...
    pub async fn commit(&self) -> CommitResult {
        match self {
            DatabaseTabTxn::MemTabTxn(txn) => txn.commit().await,
            DatabaseTabTxn::LogFileTabTxn(txn) => txn.commit().await,
        }
    }
    ...
    pub async fn rollback(&self) -> DBResult {
        match self {
            DatabaseTabTxn::MemTabTxn(txn) => txn.rollback().await,
            DatabaseTabTxn::LogFileTabTxn(txn) => txn.rollback().await,
        }
    }
    ...
    pub async fn key_lock(
        ...
        match self {
            DatabaseTabTxn::MemTabTxn(txn) => txn.key_lock(_arr, _lock_time, _readonly).await,
            DatabaseTabTxn::LogFileTabTxn(txn) => txn.key_lock(_arr, _lock_time, _readonly).await,
        }
    }
    ...
    pub async fn query(
        ...
        match self {
            DatabaseTabTxn::MemTabTxn(txn) => txn.query(arr, _lock_time, _readonly).await,
            DatabaseTabTxn::LogFileTabTxn(txn) => txn.query(arr, _lock_time, _readonly).await,
        }
    }
    ...
    pub async fn modify(
        ...
        match self {
            DatabaseTabTxn::MemTabTxn(txn) => txn.modify(arr, _lock_time, _readonly).await,
            DatabaseTabTxn::LogFileTabTxn(txn) => txn.modify(arr, _lock_time, _readonly).await,
        }
    }
    ...
    pub async fn iter(
        ...
        match self {
            DatabaseTabTxn::MemTabTxn(txn) => txn.iter(tab, key, descending, filter).await,
            DatabaseTabTxn::LogFileTabTxn(txn) => txn.iter(tab, key, descending, filter).await,
        }
    }
    ...
    pub async fn key_iter(
        ...
        match self {
            DatabaseTabTxn::MemTabTxn(txn) => txn.key_iter(key, descending, filter).await,
            DatabaseTabTxn::LogFileTabTxn(txn) => txn.key_iter(key, descending, filter).await,
        }
    }
    ...
    pub async fn tab_size(&self) -> SResult<usize> {
        match self {
            DatabaseTabTxn::MemTabTxn(txn) => txn.tab_size().await,
            DatabaseTabTxn::LogFileTabTxn(txn) => txn.tab_size().await,
        }
    }
    ...
    pub async fn fork_rollback(&self) -> DBResult {
        match self {
            DatabaseTabTxn::MemTabTxn(_) => unimplemented!(),
            DatabaseTabTxn::LogFileTabTxn(txn) => txn.fork_rollback().await,
        }
    }
    ...
    pub async fn force_fork(&self) -> std::io::Result<usize> {
        match self {
            DatabaseTabTxn::MemTabTxn(_) => unimplemented!(),
            DatabaseTabTxn::LogFileTabTxn(txn) => txn.force_fork().await,
        }
    }
}
...
impl DatabaseMetaTxn {
    ...
    async fn alter(&self, tab_name: &Atom, meta: Option<TabMeta>) -> DBResult {
        match self {
            DatabaseMetaTxn::MemMetaTxn(txn) => txn.alter(tab_name, meta).await,
            DatabaseMetaTxn::LogFileMetaTxn(txn) => txn.alter(tab_name, meta).await,
        }
    }
    ...
    async fn snapshot(&self, _tab: &Atom, _from: &Atom) -> DBResult {
        match self {
            DatabaseMetaTxn::MemMetaTxn(txn) => txn.snapshot(_tab, _from).await,
            DatabaseMetaTxn::LogFileMetaTxn(txn) => txn.snapshot(_tab, _from).await,
        }
    }
    ...
    async fn rename(&self, _tab: &Atom, _new_name: &Atom) -> DBResult {
        match self {
            DatabaseMetaTxn::MemMetaTxn(txn) => txn.rename(_tab, _new_name).await,
            DatabaseMetaTxn::LogFileMetaTxn(txn) => txn.rename(_tab, _new_name).await,
        }
    }
    ...
    async fn prepare(&self, _timeout: usize) -> DBResult {
        match self {
            DatabaseMetaTxn::MemMetaTxn(txn) => txn.prepare(_timeout).await,
            DatabaseMetaTxn::LogFileMetaTxn(txn) => txn.prepare(_timeout).await,
        }
    }
    ...
    async fn commit(&self) -> CommitResult {
        match self {
            DatabaseMetaTxn::MemMetaTxn(txn) => txn.commit().await,
            DatabaseMetaTxn::LogFileMetaTxn(txn) => txn.commit().await,
        }
    }
    ...
    async fn rollback(&self) -> DBResult {
        match self {
            DatabaseMetaTxn::MemMetaTxn(txn) => txn.rollback().await,
            DatabaseMetaTxn::LogFileMetaTxn(txn) => txn.rollback().await,
        }
    }
}
...
impl Tr {
    ...
    pub async fn fork_tab(
        ...
        if let Some(_) =
            self.fork_txns
                .get(&(ware.clone(), tab_name.clone(), fork_tab_name.clone()))
        {
        ...
        self.fork_txns
            .insert((ware, tab_name, fork_tab_name), (new_meta, txn));
    ...
    pub async fn prepare(&mut self) -> DBResult {
        ...
        for (k, v) in self.fork_txns.iter() {
            match v
                .1
                .fork_prepare(k.0.clone(), k.1.clone(), k.2.clone(), v.0.clone())
                .await
            {
                Ok(_) => {
                    if count.fetch_sub(1, Ordering::SeqCst) == 1 {
    ...
    pub async fn commit(&mut self) -> DBResult {
        ...
        for (k, v) in self.fork_txns.iter() {
            match v
                .1
                .fork_commit(k.0.clone(), k.1.clone(), k.2.clone(), v.0.clone())
                .await
            {
                Ok(_) => {
                    if count.fetch_sub(1, Ordering::SeqCst) == 1 {
        ...
        for ((ware_name, tab_name), val) in map.into_iter() {
            ...
            match self.build(&ware_name, &tab_name).await {
                Ok(t) => match t.query(tkv, lock_time, read_lock).await {
                    Ok(vec) => match merge_result(&rvec, vec).await {
                        Ok(v) => {
    ...
    pub async fn iter(
        ...
        match self.build(&ware, &tab).await {
            Ok(t) => match t.iter(&tab, key, descending, filter).await {
                Ok(iter) => {
                    self.state = TxState::Ok;
                    Ok(iter)
                }
    ...
    pub async fn key_iter(
        ...
        self.state = TxState::Doing;
        match self.build(&ware, &tab).await {
            Ok(t) => match t.key_iter(key, descending, filter).await {
                Ok(iter) => {
                    self.state = TxState::Ok;
                    Ok(iter)
                }
    ...
    pub async fn alter(
        ...
        let ware = match self.ware_log_map.get(ware_name) {
            Some(w) => match w.check(tab_name, &meta) {
                // 检查
                Ok(_) => {
        ...
        let txn = self
            .meta_txns
            .entry(ware_name.clone())
            .or_insert_with(|| ware.meta_txn(&id))
            .clone();
...
fn tab_map(mut arr: Vec<TabKV>) -> XHashMap<(Atom, Atom), Vec<TabKV>> {
    ...
    while len > 0 {
        ...
        len -= 1;
        let r = map
            .entry((tk.ware.clone(), tk.tab.clone()))
            .or_insert(Vec::new());
        r.push(tk);
    }
    return map;
...
async fn merge_result(
    rvec: &Arc<Mutex<(usize, Vec<TabKV>)>>,
    vec: Vec<TabKV>,
) -> SResult<Vec<TabKV>> {
